...
public:
  ...
  bool is_alternating() const {
    bool disk_alt = true;
    for (size_t i = 0; i < _colors.size()-1; i += 1)
    {
        //check if next in list is not equal to what is in list
  ...
  bool is_sorted() const {
    bool disk_sorted = true;
    //checks first n elements if they are light
    for (size_t i = 0; i < light_count-1; i += 1)
    {
    ...
    //checks last n elements if they are dark
    for (size_t i = light_count; i < _colors.size()-1; i += 1)
    {
...
sorted_disks sort_left_to_right(const disk_state& before) {
  ...
  for (size_t i = 0; i < _colors.size()-1; i += 1)
  {
      if (_colors[i] == DISK_DARK)
      {
          //iterats nested loop to check for swapping condition
          for (size_t j = 0; j < _colors.size()-1; j += 1)
          {
              if(_colors[j] != _colors[j+1])
              {
                  _colors[j] = DISK_LIGHT;
                  _colors[j+1] = DISK_DARK;
              }
          }
      }
  }
...
sorted_disks sort_lawnmower(const disk_state& before) {
  ...
  for (size_t i = 0; i < _colors.size()-1; i += 1)
  {
      if (_colors[i] == DISK_DARK)
      {
          //iterats nested loop to the right to check for swapping condition
          for (size_t j = 0; j < _colors.size()-1; j += 1)
          {
              if(_colors[j] != _colors[j+1])
              {
          ...
          //iterats nested loop to the left to check for swapping condition
          for (size_t k = _colors.size()-1; k > 0; k -= 1)
          {
              if(_colors[k] != _colors[k+1])
              {
                  _colors[k] = DISK_LIGHT;
                  _colors[k-1] = DISK_DARK;
              }
          }
      }
  }
